...
public class GetSyncHeartRateTool {
    ...
    public GetSyncHeartRateTool(RestClient smartringRestClient, SmartRingApiProperties properties) {
    ...
    public ExternalAppPageHeartRateResponseDto getHeartRate(
        ...
        logger.info("Запрос данных синхронизации пульса: uuid={}, from={}, to={}, page={}, pageSize={}",
        ...
        try {
            URI uri = UriComponentsBuilder.fromUriString(properties.getBaseUrl() + SmartRingConstants.API_PREFIX + "/heartrate")
                    .queryParams(queryParams)
                    .build()
                    .toUri();

            ExternalAppPageHeartRateResponseDto response = restClient.get()
                    .uri(uri)
            ...
            logger.error("Неожиданная ошибка при получении данных пульса: {}", e.getMessage(), e);
