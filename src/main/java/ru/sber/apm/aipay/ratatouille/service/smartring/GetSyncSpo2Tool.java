...
@Service
public class GetSyncSpo2Tool {

    private static final Logger logger = LoggerFactory.getLogger(GetSyncSpo2Tool.class);

    private final RestClient restClient;
    private final SmartRingApiProperties properties;

    public GetSyncSpo2Tool(RestClient smartringRestClient, SmartRingApiProperties properties) {
    ...
    public ExternalAppPageSpo2ResponseDto getSpO2(
        ...
        try {
            URI uri = UriComponentsBuilder.fromUriString(properties.getBaseUrl() + SmartRingConstants.API_PREFIX + "/spo2")
            ...
            logger.info("Ответ синхронизации SpO2: записей={}, страница={}/{}, тело ответа={}",
            ...
            logger.error("Неожиданная ошибка при получении данных SpO2: {}", e.getMessage(), e);
