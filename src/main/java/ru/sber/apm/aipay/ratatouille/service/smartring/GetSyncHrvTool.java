...
@Service
public class GetSyncHrvTool {

    private static final Logger logger = LoggerFactory.getLogger(GetSyncHrvTool.class);

    private final RestClient restClient;
    private final SmartRingApiProperties properties;

    public GetSyncHrvTool(RestClient smartringRestClient, SmartRingApiProperties properties) {
        this.restClient = smartringRestClient;
        this.properties = properties;
    }

    @McpTool(description = "Получить данные синхронизации вариабельности сердечного ритма (HRV) с Smart Ring")
    public ExternalAppPageHrvResponseDto getHrv(
        ...
        logger.info("Запрос данных синхронизации HRV: from={}, to={}, page={}, pageSize={}", from, to, page, pageSize);
        ...
        try {
            URI uri = UriComponentsBuilder.fromUriString(properties.getBaseUrl() + SmartRingConstants.API_PREFIX + "/hrv")
                    .queryParams(queryParams)
                    .build()
                    .toUri();

            ExternalAppPageHrvResponseDto response = restClient.get()
                    .uri(uri)
                    .header(HttpHeaders.AUTHORIZATION, "Bearer " + properties.getUuid())
                    .retrieve()
                    .body(ExternalAppPageHrvResponseDto.class);

            logger.info("Ответ синхронизации HRV: записей={}, страница={}/{}, тело ответа={}",
            ...
            handleHttpError(e);
            throw SmartRingApiException.internalError("HTTP ошибка: " + e.getStatusCode(), e);
        } catch (ResourceAccessException e) {
            handleConnectionError(e);
            throw SmartRingApiException.connectionError("Ошибка соединения: " + e.getMessage(), e);
        } catch (Exception e) {
            logger.error("Неожиданная ошибка при получении данных HRV: {}", e.getMessage(), e);
