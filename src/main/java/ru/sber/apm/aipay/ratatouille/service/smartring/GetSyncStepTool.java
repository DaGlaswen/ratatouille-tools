...
@Service
public class GetSyncStepTool {

    private static final Logger logger = LoggerFactory.getLogger(GetSyncStepTool.class);

    private final RestClient restClient;
    private final SmartRingApiProperties properties;

    public GetSyncStepTool(RestClient smartringRestClient, SmartRingApiProperties properties) {
        this.restClient = smartringRestClient;
        this.properties = properties;
    }

    @McpTool(description = "Получить данные синхронизации шагов (Step) с Smart Ring")
    public ExternalAppPageStepResponseDto getSteps(
        ...
        logger.info("Запрос данных синхронизации шагов: from={}, to={}, page={}, pageSize={}", from, to, page, pageSize);
        ...
        try {
            URI uri = UriComponentsBuilder.fromUriString(properties.getBaseUrl() + SmartRingConstants.API_PREFIX + "/step")
                    .queryParams(queryParams)
                    .build()
                    .toUri();

            ExternalAppPageStepResponseDto response = restClient.get()
                    .uri(uri)
                    .header(HttpHeaders.AUTHORIZATION, "Bearer " + properties.getUuid())
                    .retrieve()
                    .body(ExternalAppPageStepResponseDto.class);

            logger.info("Ответ синхронизации шагов: записей={}, страница={}/{}, тело ответа={}",
            ...
            logger.error("Неожиданная ошибка при получении данных шагов: {}", e.getMessage(), e);
