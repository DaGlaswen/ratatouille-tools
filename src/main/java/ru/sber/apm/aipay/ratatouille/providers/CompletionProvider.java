...
public class CompletionProvider {
	...
	/**
	 * Метод завершения для имен пользователей в запросе статуса пользователя
     * URI COMPLETION
	 */
	...
	public List<String> completeUsername(String usernamePrefix) {
		...
		if (prefix.isEmpty()) {
			return List.of("Введите имя пользователя");
		}
	...
	/**
	 * Метод завершения для названий стран в запросе путешествия.
	 */
	@McpComplete(prompt = "travel-planner")
	public CompleteResult completeCountryName(CompleteRequest request) {
		String prefix = request.argument().value().toLowerCase();
		if (prefix.isEmpty()) {
			return new CompleteResult(new CompleteCompletion(List.of("Введите название страны"), 1, false));
		}
