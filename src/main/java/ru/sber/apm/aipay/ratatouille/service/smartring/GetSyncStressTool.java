...
@Service
public class GetSyncStressTool {

    private static final Logger logger = LoggerFactory.getLogger(GetSyncStressTool.class);

    private final RestClient restClient;
    private final SmartRingApiProperties properties;

    public GetSyncStressTool(RestClient smartringRestClient, SmartRingApiProperties properties) {
    ...
    public ExternalAppPageStressResponseDto getStress(
        ...
        try {
            URI uri = UriComponentsBuilder.fromUriString(properties.getBaseUrl() + SmartRingConstants.API_PREFIX + "/stress")
            ...
            logger.info("Ответ синхронизации стресса: записей={}, страница={}/{}, тело ответа={}",
            ...
            logger.error("Неожиданная ошибка при получении данных стресса: {}", e.getMessage(), e);
